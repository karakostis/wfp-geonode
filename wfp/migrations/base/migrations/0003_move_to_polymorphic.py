...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for layer in orm['layers.layer'].objects.all():
            print layer
            layer.polymorphic_ctype_id = ct.id
            layer.save()
            
        ct = ContentType.objects.get(app_label="maps", model="map")
        for m in orm['maps.map'].objects.all():
            print m
            m.polymorphic_ctype_id = ct.id
            m.save()
            
        ct = ContentType.objects.get(app_label="documents", model="document")
        for document in orm['documents.document'].objects.all():
            print document
            document.polymorphic_ctype_id = ct.id
            document.save()
    ...
    models = {
        ...
        u'layers.layer': {
            ...
            'typename': ('django.db.models.fields.CharField', [], {'max_length': '128', 'unique': 'True', 'null': 'True', 'blank': 'True'}),
    ...
    complete_apps = ['layers', 'documents', 'maps', 'base']
    symmetrical = True
