...
src_cur.execute("select resource_id, contact_id, role_id from base_contactrole where contact_id in (select id from auth_user)")
...
for src_row in src_cur:
    ...
    #user_id
    contact_id = utils.get_userid_by_oldid(src_row[1])
    #primary
    role_id = src_row[2]
    if role_id == 1:
